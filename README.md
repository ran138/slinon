...
⁧המאגר כולל אתר שיווק, עמוד מוצר נוסף, אב־טיפוס עצמאי של ממשק המשתמש ויישום מקומי מלא בשם חסכהון.⁩
...
⁧ד. ⁦haskahon⁩ — יישום המוצר המלא. זהו יישום ⁦Next.js⁩ מקומי בעברית, הכולל ממשק משתמש, נתיבי שרת, מסד נתונים מקומי, מחקר בעזרת ⁦OpenAI⁩ ויצירת קובצי שמע.⁩
...
├── haskahon/                     # Full-stack local application
...
⁧חסכהון הוא יישום מודולרי יחיד המבוסס על ⁦Next.js 16⁩, ⁦React⁩, ⁦TypeScript⁩ ו־⁦SQLite⁩. הוא מאזין כברירת מחדל רק למחשב המקומי בכתובת הבאה:⁩
...
⁧המידע המתמשך של חסכהון נשמר מקומית ואינו אמור להיכנס לבקרת הגרסאות:⁩

```text
haskahon/data/haskahon.sqlite
haskahon/data/audio/
haskahon/.env.local
```
...
⁧הגדרת ⁦Vercel⁩ המקומית מקשרת את הפרויקט לתיקיית ⁦haskahon⁩. עם זאת, היישום משתמש כרגע במסד ⁦SQLite⁩ ובקובצי שמע הנשמרים בדיסק המקומי. לפני פריסה בסביבה שבה מערכת הקבצים אינה מתמשכת, צריך להעביר את הנתונים והקבצים לאחסון מתמשך מתאים.⁩
