...
const configuredDataDir = process.env.VESTORY_DATA_DIR?.trim();
const dataDir = configuredDataDir
  ? path.resolve(configuredDataDir)
  : process.env.VERCEL
    ? path.join("/tmp", "vestory")
    : path.join(process.cwd(), "data");
fs.mkdirSync(path.join(dataDir, "audio"), { recursive: true });
const cache = globalThis as typeof globalThis & { __vestorySqlite?: Database.Database };
const sqlite = cache.__vestorySqlite ?? new Database(path.join(dataDir, "vestory.sqlite"));
if (process.env.NODE_ENV !== "production") cache.__vestorySqlite = sqlite;
