      ...
      <section className="today-quick" aria-labelledby="today-quick-heading"><div className="today-section-heading"><h2 id="today-quick-heading">תובנות מהירות</h2><button onClick={() => onNav("settings-portfolio")}>המעקב שלי <ArrowUpRight size={16}/></button></div>
        <div className="today-quick-grid" dir="ltr">
          {assets.map((asset) => {
            ...
            return <article key={asset.id} className="today-card today-asset" dir="rtl"><div className="today-asset-heading"><span className="today-asset-icon" dir="ltr">{asset.ticker === "SPY" ? "S&P" : asset.ticker.slice(0,4)}</span><div><h3><bdi dir="auto">{asset.name || asset.ticker}</bdi></h3><bdi dir="ltr">{asset.ticker}</bdi></div><span className="today-asset-change" aria-label="שינוי באחוזים אינו זמין">—</span></div><div className="today-asset-body"><div className="today-trend-empty" aria-label="נתוני מגמה אינם זמינים"><svg viewBox="0 0 110 65" aria-hidden="true"><defs><linearGradient id={`trend-${asset.id}`} x2="0" y2="1"><stop stopColor="#405ee4" stopOpacity=".2"/><stop offset="1" stopColor="#405ee4" stopOpacity="0"/></linearGradient></defs><rect width="110" height="65" fill={`url(#trend-${asset.id})`}/><path d="M0 18H110 M0 42H110 M20 0V65 M65 0V65" stroke="#557bb7" strokeOpacity=".15"/><path d="M8 33H102" stroke="#7194c8" strokeDasharray="3 5" strokeOpacity=".4"/></svg><small>אין נתוני שוק</small></div><p>{related?.title ?? "נכס במעקב שלך. עדכוני השוק יוצגו כשיהיה מקור מאומת."}</p></div>{related && <button className="today-asset-link" onClick={() => onNav("player")}><Sparkles size={15}/>למה זה בפודקאסט?</button>}</article>;
          })}
