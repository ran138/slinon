...
export async function sendPodcastReadyEmail(params: { to: string; briefId: string; title: string | null }) {
  ...
  const supportEmail = "customer.service@slinon.me";
  ...
  try {
    await resend.emails.send({
      ...
      to: params.to,
      subject,
      // A plain-text alternative and a List-Unsubscribe header are both
      // well-known signals mailbox providers use to distinguish real
      // transactional mail from spam — worth having even though this isn't
      // technically a bulk/marketing send.
      text: [
        "הפודקאסט שלך מוכן להאזנה",
        params.title ?? "",
        "",
        `להאזנה: ${listenUrl}`,
        "",
        `לא רוצים לקבל את ההודעות האלה? אפשר לכבות אותן בהעדפות באפליקציה, או לפנות אלינו: ${supportEmail}`,
      ].filter(Boolean).join("\n"),
      headers: {
      ...
      html: `
        <div dir="rtl" lang="he" style="font-family:Heebo,Arial,sans-serif;background:#080910;color:#f7f7fb;padding:32px 24px;">
          ...
          <img src="${appUrl}/assets/slinon-logo-transparent.png" alt="Slinon" width="80" style="display:block;margin:24px auto 0;opacity:0.9;" />
        </div>
      `,
    });
  } catch (error) {
